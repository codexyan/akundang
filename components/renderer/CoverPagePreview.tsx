...
import type { TemplateRecord, NewInvitationData } from '@/lib/types'
...
interface Props {
  ...
  /** Increment untuk memicu ulang animasi dekorasi */
  decorPreviewKey?: number
}

export default function CoverPagePreview({ template, data, previewGuestName, containerHeight, decorPreviewKey }: Props) {
  const { config } = template
  const { meta, opening } = config
  ...
  const gradStop    = Math.round(gradH * 0.55)
  ...
  return (
    <div style={{
      ...
      {/* Dekorasi aset custom — key berubah = animasi diulang */}
      ...
      <div style={{
        ...
        {/* Tombol */}
        {/* Skip button (opsional — auto-dismiss timer berjalan) */}
        <div style={{
