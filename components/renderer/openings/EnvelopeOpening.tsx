...
export default function EnvelopeOpening({ config, data, meta, onOpen, positionMode = 'fixed' }: Props) {
  ...
  /* Envelope outline geometry — thin lines only */
  const envW = 220
  const envH = 150
  const flapH = 70
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-[8] flex items-center justify-center pointer-events-none">
        <motion.svg
          width={envW} height={envH + flapH}
          viewBox={`0 0 ${envW} ${envH + flapH}`}
          fill="none"
        ...
        >
          {/* Envelope body */}
          <rect x="1" y={flapH} width={envW - 2} height={envH - 1}
            stroke={accent} strokeWidth="1" rx="3" />
          {/* Flap triangle */}
          <polyline
            points={`1,${flapH} ${envW / 2},${flapH + flapH * 0.85} ${envW - 1},${flapH}`}
            stroke={accent} strokeWidth="0.8" />
          {/* Top flap */}
          <polyline
            points={`1,${flapH} ${envW / 2},2 ${envW - 1},${flapH}`}
            stroke={accent} strokeWidth="1" />
          {/* Seal circle */}
          <circle cx={envW / 2} cy={flapH + 10} r="12"
            stroke={accent} strokeWidth="0.8" />
          <circle cx={envW / 2} cy={flapH + 10} r="6"
